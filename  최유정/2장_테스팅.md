...
# 스프링 테스트
## JUnit에서의 테스트 클래스와 애플리케이션 컨텍스트 관리

JUnit이 매번 테스트 클래스의 오브젝트를 새로 만든다는 문제를 해결하기 위해 여러 방법이 제시되어 있다.

## 문제 인식

JUnit은 테스트 클래스 전체에 걸쳐 딱 한 번만 실행되는 `@BeforeClass` 스태틱 메소드를 지원한다. 이 메소드에서 애플리케이션 컨텍스트를 만들어 스태틱 변수에 저장하고, 테스트 메소드에서 사용할 수 있도록 할 수 있다.

그러나 스프링이 제공하는 애플리케이션 컨텍스트 테스트 지원 기능을 사용하는 것이 더 편리하다.

## 스프링 테스트 컨텍스트 프레임워크 적용

`@ExtendWith(SpringExtension.class)`과 `@ContextConfiguration`를 이용하여 테스트에서 사용할 `ApplicationContext`를 만들고 공유한다.
...
이렇게 하면 테스트 클래스 전체에서 하나의 애플리케이션 컨텍스트를 사용한다..

## 테스트 메소드의 컨텍스트 공유

`@ExtendWith(SpringExtension.class)`를 사용하면 테스트 클래스 간에도 애플리케이션 컨텍스트가 공유된다. 두 테스트 클래스가 같은 설정 파일을 사용하는 경우에 해당
...
이 경우에는 두 테스트 클래스가 한 애플리케이션 컨텍스트를 공유한다.

## DI와 테스트

### 테스트 코드에 의한 DI

테스트할 때 운영용 `DataSource`를 사용하지 않고, 테스트 코드에서 테스트용 `DataSource`를 만들어서 사용.
...
public class UserDaoTest {
    ...
    @BeforeEach
    public void setUp() {
        // 운영용 DataSource 대신 테스트용 DataSource를 생성하여 DI
...
이렇게 함으로써 운영과는 별개의 테스트용 `DataSource`를 사용하여 테스트를 진행.

### @DirtiesContext

`@DirtiesContext` 애노테이션은 해당 테스트 클래스나 메소드에서 애플리케이션 컨텍스트의 상태를 변경한다는 것을 알려준다.
특히, 테스트에서 빈의 의존관계를 강제로 DI 하는 경우에 사용된다.
...
이를 통해 테스트 클래스 간에 애플리케이션 컨텍스트를 공유하지 않도록 할 수 있다.

## 테스트를 위한 별도의 DI 설정

테스트에서 사용될 `DataSource` 클래스가 빈으로 정의된 테스트 전용 설정 파일을 만들어 사용할 수도 있다.

```java
@ExtendWith(SpringExtension.class)
@ContextConfiguration(locations = { "/spring/applicationContext.xml", "/spring/testContext.xml" })
public class UserDaoTest {
    //...
}
```

이렇게 하면 두 가지 종류의 설정 파일을 만들어서 테스트 전용 설정 파일에서는 테스트에 적합한 설정을 사용하게 된다.

## 컨테이너 없는 DI 테스트

만약 스프링 컨테이너를 사용하지 않고 테스트를 진행하고 싶다면, 스프링 컨테이너를 사용하지 않는 방법도 있다.
...
이렇게 하면 스프링 컨테이너를 사용하지 않고도 테스트를 진행할 수 있다.

## 비침투적 기술

애플리케이션 로직을 담은 코드에 아무런 영향을 주지 않고 적용이 가능한 기술